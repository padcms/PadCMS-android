...
public class ImageViewTouchBase extends ImageViewResources {
	...
	protected static int borderCount = 0;
	...
	public boolean isPosibleScrollImageToLeft(float x, float y) {
		RectF rect = getBitmapRect();
	...
	public boolean isPosibleScrollImageToRight(float x, float y) {
		RectF rect = getBitmapRect();
	...
	protected void updateRect(RectF bitmapRect, RectF scrollRect) {
		...
		// Log.e( LOG_TAG, "borderCount " + borderCount );
		if (bitmapRect.left > -1 || bitmapRect.right <= width + 1) {
	...
	/*
	 * protected void scrollBy( float distanceX, float distanceY, final float
	 * durationMs ) { final float dx = distanceX; final float dy = distanceY;
	 * final long startTime = System.currentTimeMillis(); mHandler.post( new
	 * Runnable() {
	 * 
	 * float old_x = 0; float old_y = 0;
	 * 
	 * public void run() { long now = System.currentTimeMillis(); float
	 * currentMs = Math.min( durationMs, now - startTime ); // float x =
	 * Cubic.easeOut( currentMs, 0, dx, durationMs ); // float y =
	 * Cubic.easeOut( currentMs, 0, dy, durationMs ); // panBy( ( x - old_x ), (
	 * y - old_y ) ); // old_x = x; // old_y = y; if ( currentMs < durationMs )
	 * { mHandler.post( this ); } else { RectF centerRect = getCenter( true,
	 * true ); if ( centerRect.left != 0 || centerRect.top != 0 ) scrollBy(
